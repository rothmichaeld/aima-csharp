...
namespace aima.core.logic.fol.kb
{
    ...
    {
        private readonly FOLParser parser;
        private InferenceProcedure inferenceProcedure;
        private readonly Unifier unifier;
        private readonly SubstVisitor substVisitor;
        private readonly VariableCollector variableCollector;
        private readonly StandardizeApart _standardizeApart;
        private readonly CNFConverter cnfConverter;

        // Persistent data structures	
        // Keeps track of the Sentences in their original form as added to the
        // Knowledge base.
        private readonly List<Sentence> originalSentences = new List<Sentence>();
        ...
        private readonly List<Clause> allDefiniteClauses = new List<Clause>();
        private readonly List<Clause> implicationDefiniteClauses = new List<Clause>();
        // All the facts in the KB indexed by Atomic Sentence name (Note: pg. 279)
        private readonly Dictionary<String, List<Literal>> indexFacts = new Dictionary<String, List<Literal>>();
        // Keep track of indexical keys for uniquely standardizing apart sentences
        private readonly StandardizeApartIndexical variableIndexical = StandardizeApartIndexicalFactory
            .newStandardizeApartIndexical('v');
        private readonly StandardizeApartIndexical queryIndexical = StandardizeApartIndexicalFactory
        ...
        {
            parser = new FOLParser(new FOLDomain(domain));
            this.inferenceProcedure = inferenceProcedure;
            this.unifier = unifier;
            //
            substVisitor = new SubstVisitor();
            variableCollector = new VariableCollector();
            _standardizeApart = new StandardizeApart(variableCollector,
                substVisitor);
            cnfConverter = new CNFConverter(parser);
        }
        ...
        public void clear()
        {
            ...
            clauses.Clear();
            allDefiniteClauses.Clear();
            implicationDefiniteClauses.Clear();
            indexFacts.Clear();
        }
